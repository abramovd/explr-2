...
class ExplorationDB(object):
    ...
    def __init__(self, context, implications, cond=lambda x: True):
        ...
        self._cxt_object_implications = context.get_object_implications(
                                                confirmed=self._implications,
                                                cond=cond)
    ...
    def add_example(self, name, intent):
        ...
        print 'after adding'
        print self._cxt_object_implications
    ...
    @context_modifier
    def add_attribute_example(self, name, extent):
    ...
    def get_open_object_implications(self):
        return copy(self._cxt_object_implications)
    ...
    open_object_implications = property(get_open_object_implications)
...
class ObjectExploration(object):
    """Exploration"""
    def __init__(self, db, expert):
        super(ObjectExploration, self).__init__()
        self.db = db
        self.expert = expert
    ...
    def reject_implication(self, imp):
        """docstring for reject_implication"""
        examples, extents = self.expert.provide_object_counterexample(imp)
        # TODO: refactor
        if type(extents) == set:
            examples, extents = [examples], [extents]
        #if imp.is_respected(intents[0]):
        #    raise NotCounterexample()
        for i in range(len(examples)):
            self.db.add_attribute_example(examples[i], extents[i])
        self.db.complete()
    ...
    def get_open_implications(self):
        return self.db.open_object_implications