...
class ObjectImplication(models.Model):
    """Implication on attributes in background knowledge"""
    premise = models.ManyToManyField(FObject, related_name='premise_set+')
    conclusion = models.ManyToManyField(FObject, related_name='conclusion_set+')
    minimal_generator = models.ManyToManyField(FObject, related_name='mingen_set+')
    ...
    def get_premise(self):
        return set([obj.pk for obj in self.premise.all()])

    def get_conclusion(self):
        return set([obj.pk for obj in self.conclusion.all()])

    def get_as_fca_implication(self):
        return fca.Implication(set([obj.name for obj in self.premise.all()]),
                               set([obj.name for obj in self.conclusion.all()]))