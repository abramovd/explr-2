...
from exploration import ExplorationWrapper, ObjectExplorationWrapper
...
def implications(request, template_name="exploration/implications.html"):
    ...
    confirmed_implications = list(ExplorationWrapper.get_background_knowledge(group))
    attributes = group.content_objects(FAttribute)
    objects = group.content_objects(FObject)
    print open_implications

    open_object_implications = list(ObjectExplorationWrapper.get_open_implications(group))
    # open_implications = [open_implications_generator.next() for _ in xrange(10)]
    confirmed_object_implications = list(ObjectExplorationWrapper.get_background_knowledge(group))
...
@login_required
def get_object_premise(request):
    """AJAX"""
    group, bridge = group_and_bridge(request)
    ...
    if request.method == 'POST':
        imp_pk = request.POST['imp_pk']
        try:
            objects_ids = ObjectExplorationWrapper.get_premise(group, int(imp_pk))
        except:
            return HttpResponse("reload")

        return HttpResponse(simplejson.dumps(list(objects_ids), ensure_ascii=False), mimetype='application/json')
    else:
        raise Http404
...
def get_object_conclusion(request):
    ...
    if request.method == 'POST':
        imp_pk = request.POST['imp_pk']
        try:
            objects_ids = ObjectExplorationWrapper.get_conclusion(group, int(imp_pk))
        except:
            return HttpResponse("reload")
...
def confirm_object_implication(request):
    ...
    if request.method == 'POST':
        pk = request.POST['pk']
        try:
            implication = ObjectExplorationWrapper.confirm_implication(group, int(pk))
        except KeyError:
            # TODO: Relative basis was changed. Show message.
            pass
        else:
...
def unconfirm_object_implication(request):
    ...
    if request.method == 'POST':
        pk = request.POST['pk']
        implication = ObjectImplication.objects.get(pk=pk).get_as_fca_implication()
