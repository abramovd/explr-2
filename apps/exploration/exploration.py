...
class ObjectImplicationsContainer(object):
    ...
    def _get_implications(self):
        return self.group.content_objects(ObjectImplication)
...
class BackgroundObjectKnowledgeList(ObjectImplicationsContainer):
    """Interface for background knowledge implications"""
    ...
    def append(self, fca_imp):
        db_imp = ObjectImplication()
        self.group.associate(db_imp, commit=False)
        db_imp.save()

        get_obj = lambda name: self.group.content_objects(FObject).get(pk=name)
        db_imp.premise.add(*[get_obj(name) for name in fca_imp.premise])
        db_imp.conclusion.add(*[get_obj(name) for name in fca_imp.conclusion])
...
class DBContext(fca.Context):
    ...
    def set_attribute_extent(self, extent, name):
        ...
        attr.objects.clear()
        for attr in extent:
            attr.objects.add(self.group.content_objects(FObject).get(pk=attr))
...
class WebExpert(object):
    ...
    def set_object_counterexample(self, example, extent):
        self.example_attr = example
        self.extent = extent
...
class WebObjectExploration(ObjectExploration):

    def __init__(self, group):
        db = ExplorationDB(DBContext(group), BackgroundObjectKnowledgeList(group))
        expert = WebExpert()
        super(WebObjectExploration, self).__init__(db, expert)

    def confirm_implication(self, imp_pk):
        imp = self.db.open_object_implications[imp_pk]
        super(WebObjectExploration, self).confirm_implication(imp)
        return imp

    def unconfirm_implication(self, imp_pk):
        super(WebObjectExploration, self).unconfirm_implication(ObjectImplication.objects.get(pk=imp_pk))

    def reject_implication(self, imp_pk):
        imp = self.db.open_object_implications[imp_pk]
        super(WebObjectExploration, self).reject_implication(imp)
...
class ExplorationWrapper(object):
    ...
    def get_open_implications(cls, group):
        ...
        # in this module)
        #print list(cls.get_exploration(group).db.open_implications.get_query_set(attributes))
    ...
    @classmethod
    def delete_attribute(cls, group, attribute):
        cls.get_exploration(group).db.delete_attribute(attribute)
        cls.save_lattice(group)

    @classmethod
    def delete_object(cls, group, object):
        cls.get_exploration(group).db.delete_example(object)
        cls.save_lattice(group)
...
class ObjectExplorationWrapper(object):
    ...
    def get_exploration(cls, group):
        if group not in cls._explorations:
            cls._explorations[group] = WebObjectExploration(group)
        return cls._explorations[group]
    ...
    def get_open_implications(cls, group):
        ...
        # primary keys of corresponding attributes in database. Therefore we
        # need to translate them to readable format.
        objects = {obj.pk : obj.name for obj in group.content_objects(FObject)}
        # open_implications is object of class ImplicationsList (also defined
        # in this module)
        #print list(cls.get_exploration(group).db.open_object_implications.get_query_set(objects))
        return cls.get_exploration(group).db.open_object_implications.get_query_set(objects)
    ...
    @classmethod
    def get_premise(cls, group, imp_pk):
        return cls.get_exploration(group).db.open_object_implications[imp_pk].premise

    @classmethod
    def get_conclusion(cls, group, imp_pk):
        return cls.get_exploration(group).db.open_object_implications[imp_pk].conclusion
    ...
    @classmethod
    def reject_implication_with_counterexample(cls, group, imp_pk, example, extent):
        cls.get_exploration(group).expert.set_object_counterexample(example, extent)
        cls.get_exploration(group).reject_implication(imp_pk)
        cls.save_lattice(group)
    ...
    @classmethod
    def edit_attribute(cls, group, object_, extent):
        cls.get_exploration(group).db.edit_attribute(object_, object_, extent)
        cls.save_lattice(group)

    @classmethod
    def add_object(cls, group, attribute, intent):
        cls.get_exploration(group).db.add_example(attribute, intent)
        cls.save_lattice(group)
