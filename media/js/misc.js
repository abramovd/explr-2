...
$("a.confirm-obj-imp").click(function(event) {
	event.preventDefault();
	imp_pk = $(this).attr("id")
	...
	$.post("confirm_object_implication", {pk: imp_pk},
			function(data) {
				$.unblockUI();
				document.location.reload();
				//document.location.href = document.location.href;
...
$("a.unconfirm-obj-imp").click(function(event) {
	event.preventDefault();
	imp_pk = $(this).attr("id")
	...
	$.post("unconfirm_object_implication", {pk: imp_pk},
			function(data) {
				$.unblockUI();
				document.location.reload();
				//document.location.href = document.location.href;
...
$(document).ready(function() {
	...
	$("a.reject-obj-imp").each(function() {
	  var imp_pk = this.id;
	  $(this).fancybox({
	        'scrolling'		: 'no',
			'titleShow'		: false,
	        'onStart' : function() {
	        	$("#imp_obj_pk").val(imp_pk);
	        	$.post("get_object_premise", {imp_pk: imp_pk},
	        	function(data) {
                    if (data == "reload") {
                        document.location.href = document.location.href;
                    }
	        		$(".object-input").each(function(index) {
				        if (data.indexOf(parseInt($(this).attr("name"))) != -1) {
					       $(this).prop("checked", true)
					       $(this).prop("disabled", true)
				        }
                ...
                $.post("get_object_conclusion", {imp_pk: imp_pk},
                function(data) {
                    if (data == "reload") {
						//document.location.reload();
                        document.location.href = document.location.href;
